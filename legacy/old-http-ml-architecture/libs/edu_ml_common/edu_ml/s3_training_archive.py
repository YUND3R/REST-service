...
def try_archive_analyze_for_training(
    *,
    ...
    record: dict[str, Any] = {
        ...
        "report": report_json,
        "raw_model_completion": raw_model_completion or "",
    }
    key = _key_prefix("analyze", user_id)
    body = json.dumps(record, ensure_ascii=False, default=str).encode("utf-8")
    try:
        _s3_client(s.aws_region).put_object(
            Bucket=s.s3_bucket,
            Key=key,
            Body=body,
            ContentType="application/json; charset=utf-8",
            Metadata={"kind": "analyze", "user-id": user_id or ""},
        )
...
def try_archive_generate_task_for_training(
    *,
    user_id: str | None = None,
    task: dict[str, Any] | None,
    raw_completion: str,
    ...
    try:
        _s3_client(s.aws_region).put_object(
            ...
            Body=body,
            ContentType="application/json; charset=utf-8",
            Metadata={"kind": "generate_task", "user-id": user_id or ""},
        )
