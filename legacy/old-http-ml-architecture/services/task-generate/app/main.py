    ...
    if settings.redis_url:
        ...
        try:
            ...
            app.state.job_store = AsyncJobStore(
                JobRedisConfig(
                    url=settings.redis_url, key_prefix="edu:job:generate:", ttl_seconds=settings.job_result_ttl_seconds
                ),
                job_r,
            )
...
async def put_user_progress(
    req: UserProgressPut, request: Request, user_id: str = Depends(required_uid)
) -> dict[str, Any]:
    store = getattr(request.app.state, "user_store", None)
    ...
    try:
        return await store.set_user_progress(
            user_id,
            weak_tags=list(req.weak_tags),
...
async def put_user_profile(
    req: UserProfilePut, request: Request, user_id: str = Depends(required_uid)
) -> dict[str, Any]:
    store = getattr(request.app.state, "user_store", None)
    if not isinstance(store, UserRedisStore) or not store.connected:
        raise HTTPException(status_code=503, detail="Хранилище Redis не подключено (задайте REDIS_URL)")
...
async def generate_from_progress(
    ...
    if weakest:
        ...
        raise HTTPException(
            status_code=404,
            detail=(
                "Нет статистики по анализам (нужен хотя бы один /v1/analyze с авторизацией) "
                "и нет сохранённого прогресса (PUT /v1/user/progress)."
            ),
        )
    ...
    raw, task, ok = await asyncio.to_thread(
        generate_broken_task,
        tag1=t1,
        tag2=t2,
        tag3=t3,
        ...
        weights=weights,
        tests_hints=th,
        requirements_hints=rh,
        constraints_hints=ch,
    )
    try:
        await store.save_task(
            user_id, task=task, raw_completion=raw, parse_ok=ok, tag1=t1, tag2=t2, tag3=t3, difficulty=difficulty_used
        )
    except Exception as e:
    ...
    if difficulty_used in ("easy", "medium", "hard"):
        ...
        d_final = "medium"
    return GenerateFromProgressResponse(
        task=task,
        task_typed=coerce_ml_task(task),
        raw_completion=raw,
        parse_ok=ok,
        used_weak_tags=(t1, t2, t3),
        difficulty_used=d_final,
    )


@app.post("/v1/generate-task", response_model=GenerateTaskResponse)
async def generate_task(
    req: GenerateTaskRequest, request: Request, user_id: str | None = Depends(optional_uid)
) -> GenerateTaskResponse:
    raw, task, ok = await asyncio.to_thread(
        generate_broken_task,
        tag1=req.tag1,
