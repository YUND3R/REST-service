...
class UserRedisStore:
    ...
    async def save_analyze(self, user_id: str, *, analysis: str, code: str, task_context: str, task: dict[str, Any] | None=None, tag_scores: dict[str, float] | None=None, report_json: dict[str, Any] | None=None) -> dict[str, Any]:
        assert self._client is not None
        payload: dict[str, Any] = {'analysis': analysis, 'code': code, 'task_context': task_context, 'stored_at': _now_iso()}
        if task is not None:
            payload['task'] = task
    ...
    async def get_analyze_history(self, user_id: str, *, limit: int, offset: int=0) -> tuple[list[dict[str, Any]], int]:
        assert self._client is not None
        k_hist = self._k_hist_analyze(user_id)
    ...
    async def save_task(self, user_id: str, *, task: dict[str, Any] | None, raw_completion: str, parse_ok: bool, tag1: str, tag2: str, tag3: str, difficulty: str) -> dict[str, Any]:
        assert self._client is not None
        payload = {'task': task, 'raw_completion': raw_completion, 'parse_ok': parse_ok, 'request': {'tag1': tag1, 'tag2': tag2, 'tag3': tag3, 'difficulty': difficulty}, 'stored_at': _now_iso()}
        raw = json.dumps(payload, ensure_ascii=False)
    ...
    async def set_user_progress(self, user_id: str, *, weak_tags: list[str], difficulty: str, weights: tuple[float, float, float] | None, tests_hints: list[str] | None=None, requirements_hints: list[str] | None=None, constraints_hints: list[str] | None=None) -> dict[str, Any]:
        ...
        if weights is not None:
            ...
            w_payload = None
        payload: dict[str, Any] = {'weak_tags': [t1, t2, t3], 'difficulty': difficulty, 'weights': w_payload, 'updated_at': _now_iso()}
...
def merge_tag_mastery_observations_sync(redis_url: str, user_id: str, mastery: dict[str, float], *, ttl_seconds: int | None=None) -> None:
