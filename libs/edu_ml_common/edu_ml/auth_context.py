...
def _authenticate(request: Request) -> Principal:
    ...
    raise HTTPException(status_code=401, detail='Нужен JWT Bearer или X-API-Key', headers={'WWW-Authenticate': 'Bearer'})
...
def required_uid(ctx: Annotated[ApiContext, Depends(get_api_context)]) -> str:
    if not ctx.user_id:
        s = get_auth_settings()
        if s.auth_enabled:
            raise HTTPException(status_code=400, detail='Для API key укажите X-User-Id; для JWT поле sub задаёт пользователя')
