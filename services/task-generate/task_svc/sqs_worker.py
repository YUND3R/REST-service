...
def _handle_one(sqs, queue_url: str, jobs: SyncJobStore, message: dict) -> None:
    ...
    try:
        ...
        raw, task, ok = generate_broken_task(tag1=req.tag1, tag2=req.tag2, tag3=req.tag3, difficulty=req.difficulty, weights=req.weights, tests_hints=req.tests_hints, requirements_hints=req.requirements_hints, constraints_hints=req.constraints_hints)
        typed = coerce_ml_task(task)
        result = {'task': task, 'task_typed': typed.model_dump() if typed else None, 'raw_completion': raw, 'parse_ok': ok}
        jobs.mark_completed(job_id, result)
        try_archive_generate_task_for_training(user_id=body_raw.get('user_id'), task=task, raw_completion=raw, parse_ok=ok, tag1=req.tag1, tag2=req.tag2, tag3=req.tag3, difficulty=req.difficulty, job_id=job_id)
