...
def generate_broken_task(*, tag1: str, tag2: str, tag3: str, difficulty: str, weights: tuple[float, float, float] | None, tests_hints: list[str] | None=None, requirements_hints: list[str] | None=None, constraints_hints: list[str] | None=None) -> tuple[str, dict[str, Any] | None, bool]:
    ensure_model_loaded()
    ...
    parts = [f'Сгенерируй новую ML bugfix-задачу по параметрам ниже. В `broken_code` оставь ошибки и при необходимости маркер `ВОТ ТУТ НУЖНО ИСПРАВИТЬ КОД`.\n{json.dumps(payload, ensure_ascii=False, indent=2)}']
    hint_lines: list[str] = []
    if tests_hints:
    ...
    with torch.no_grad():
        out = runtime.model.generate(**inputs, max_new_tokens=runtime.settings.max_new_tokens, temperature=runtime.settings.temperature, top_p=runtime.settings.top_p, do_sample=True, pad_token_id=runtime.tokenizer.pad_token_id, eos_token_id=runtime.tokenizer.eos_token_id)
