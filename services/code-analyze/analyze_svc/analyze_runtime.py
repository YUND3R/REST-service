...
def _load_causal_lm(local_dir: str) -> tuple[Any, Any]:
    ...
    model = AutoModelForCausalLM.from_pretrained(local_dir, torch_dtype=dtype, device_map='auto', trust_remote_code=True, local_files_only=True)
...
def parse_model_completion(completion: str) -> AnalyzeResult:
    ...
    if idx2 >= 0:
        ...
        return AnalyzeResult(analysis=head or raw, tag_scores=scores, report_json=legacy_report if scores or head else None)
    return AnalyzeResult(analysis=raw, tag_scores={}, report_json=None)

def _schema_block() -> str:
    return '{\n  "task": {"id": null, "title": "…", "description": "…", "difficulty": "…", "tags": [{"name": "тег", "weight": 0.5}]},\n  "solutions": [{\n    "solution": {"code": "<код ученика>", "success": true},\n    "analysis": { ... }\n  }]\n}'

def _analyze_prompt_lines_legacy(task_context: str, code: str) -> list[str]:
    schema = '{\n  "task": {"id": null, "title": "строка", "description": "строка", "difficulty": "easy|medium|hard", "tags": [{"name": "тег", "weight": 0.5}]},\n  "solutions": [{\n    "solution": {"code": "<тот же код ученика>", "success": true},\n    "analysis": {\n      "summary": "краткое резюме для ученика",\n      "task_compliance": {\n        "is_relevant": true,\n        "score": 8,\n        "description": "строка",\n        "tag_alignment": {\n          "ИмяТегаИзУсловия": {\n            "required_weight": 0.5, "applied": true, "score": 8, "evidence": "текст"\n          }\n        },\n        "missing_requirements": [],\n        "extra_features": []\n      },\n      "code_quality_score": 8.0,\n      "correctness": {"is_correct": true, "score": 8, "edge_cases_handled": true},\n      "strengths": [], "weaknesses": [], "recommendations": [],\n      "detailed_analysis": "развёрнутый разбор",\n      "Временная сложность решения": {"time_complexity": "O(...)", "space_complexity": "O(...)", "efficiency": "...", "explanation": "..."}\n    }\n  }]\n}'
    tail_instr = 'После произвольного текстового разбора в конце ответа выведи ровно одну финальную строку: префикс ' + EDU_REPORT_JSON + ' сразу затем один валидный JSON-объект (без ```). После этой строки ничего не добавляй.'
    return ['Ты — эксперт по разбору учебных решений.', 'Сверстай поле task по приведённому ниже тексту условия (title, description, difficulty, tags с весами из текста). В solutions ровно один элемент: переданный код в solution.code, success по факту.', 'Заполни analysis.task_compliance: tag_alignment — для каждого тега из task.tags (ключ = name тега) объекты required_weight, applied, score (1..10), evidence.', 'Добавь analysis.summary, detailed_analysis, strengths, weaknesses, recommendations, correctness, code_quality_score; при необходимости блок «Временная сложность решения»; для нерелевантного ответа — task_compliance.wrong_approach (строка).', tail_instr, 'Структура JSON (ключи):', schema, '', 'Условие задачи:', task_context.strip(), '', 'Код:', f'```python\n{code}\n```']
...
def _analyze_prompt_lines_with_task(task: AnalyzeTaskInput, task_context_extra: str | None, code: str) -> list[str]:
    ...
    tail_instr = 'После произвольного текстового разбора в конце ответа выведи ровно одну финальную строку: префикс ' + EDU_REPORT_JSON + ' сразу затем один валидный JSON-объект (без ```). После этой строки ничего не добавляй.'
    lines: list[str] = ['Ты — эксперт по разбору учебных решений.', 'Ниже переданы четыре поля задания с платформы. В финальном JSON скопируй их в report.task (id, title, description, difficulty) дословно.', 'Сгенерируй сам поле report.task.tags — массив {"name", "weight"} по смыслу описания (положительные веса; сумма может быть 1).', 'Сгенерируй сам solutions и объекты analysis (summary, task_compliance, tag_alignment по именам из task.tags и т.д.). В solutions один элемент: тот же код в solution.code.', 'Заполни analysis.task_compliance.tag_alignment для каждого task.tags[].name — required_weight совпадает с weight тега, score (1..10), evidence.', 'Добавь detailed_analysis, strengths, weaknesses, recommendations, correctness, code_quality_score; при необходимости «Временная сложность решения».', tail_instr, 'Структура JSON:', _schema_block(), '', 'Входные поля задания (не меняй в ответе):', task_json]
...
def analyze_code(*, code: str, task: AnalyzeTaskInput | None=None, task_context: str | None=None) -> AnalyzeResult:
    ...
    with torch.no_grad():
        out = runtime.model.generate(**inputs, max_new_tokens=runtime.settings.max_new_tokens, do_sample=True, temperature=0.3, top_p=0.9, pad_token_id=runtime.tokenizer.pad_token_id, eos_token_id=runtime.tokenizer.eos_token_id)
    ...
    return AnalyzeResult(analysis=parsed.analysis, tag_scores=parsed.tag_scores, report_json=parsed.report_json, raw_completion=raw_completion)
