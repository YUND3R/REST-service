...
from edu_ml.schemas import AnalyzeHealthResponse, AnalyzeRequest, AnalyzeResponse, AsyncJobStatusResponse, QueuedJobAccepted, UserHistoryResponse, UserStoredEnvelope, analyze_request_storage_blob
...
async def lifespan(app: FastAPI):
    ...
    if settings.redis_url:
        ...
        try:
            ...
            app.state.job_store = AsyncJobStore(JobRedisConfig(url=settings.redis_url, key_prefix='edu:job:analyze:', ttl_seconds=settings.job_result_ttl_seconds), job_r)
...
def health(request: Request) -> AnalyzeHealthResponse:
    ...
    return AnalyzeHealthResponse(status='ok', model_loaded=runtime.model is not None, redis_connected=redis_ok, auth_enabled=get_auth_settings().auth_enabled)

@app.post('/v1/analyze', response_model=AnalyzeResponse)
async def analyze(req: AnalyzeRequest, request: Request, user_id: str | None=Depends(optional_uid)) -> AnalyzeResponse:
    ...
    if user_id and isinstance(store, UserRedisStore) and store.connected:
        try:
            await store.save_analyze(user_id, analysis=text, code=req.code, task_context=analyze_request_storage_blob(req), task=req.task.model_dump() if req.task else None, tag_scores=scores, report_json=result.report_json)
        except Exception as e:
    ...
    async def _s3_training_export() -> None:
        try:
            ...
            await asyncio.to_thread(try_archive_analyze_for_training, user_id=user_id, code=req.code, task=tdict, task_context=req.task_context, analysis=text, tag_scores=scores, report_json=result.report_json, raw_model_completion=result.raw_completion, job_id=None)
        except Exception as e:
...
async def queue_analyze(req: AnalyzeRequest, request: Request, user_id: str | None=Depends(optional_uid)) -> QueuedJobAccepted:
    settings = get_analyze_settings()
...
async def user_analyze_history(request: Request, user_id: str=Depends(required_uid), limit: int=Query(500, ge=1, le=10000), offset: int=Query(0, ge=0, le=1000000)) -> UserHistoryResponse:
    store = getattr(request.app.state, 'user_store', None)
    if not isinstance(store, UserRedisStore) or not store.connected:
        raise HTTPException(status_code=503, detail='Хранилище Redis не подключено (задайте REDIS_URL)')
